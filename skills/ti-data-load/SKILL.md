---
name: ti-data-load
description: PSA/IMA顧客orgへレコードデータを整合を保って一括で投入・移行する能力スキル。マスタ→トランザクションの順序、自動作成される会計計上レコード（売上・請求・入金予定・支払予定・支払・仕入経費）は投入せず自動作成、外部ID（tb_ExId）での組織横断の関係解決、サンドボックス間移行、期首残高移行、デモ/PoC/テストデータの自動作成チェックボックス連鎖生成、投入時の制御と前提セットアップ、よくあるインポートエラーの対処を扱う。TRIGGER when 別組織からのデータ移行（本番導入・サンドボックス間・クレンジング入れ直し）、デモ/PoC/テスト用データの作成投入、既存データの一括追加・一括更新（upsert）、稼働後のCSV/ファイルによる一括登録・一括更新、期首残高の移行、自動作成連鎖でのデモトランザクション生成、投入前の前提セットアップ。DO NOT TRIGGER when 稼働後の日常の少量（1件〜数件・対話的）なレコード作成・更新やマスタメンテは use ti-update、型・意味・書込前ゲートの参照は use ti-reference、どのデモシナリオを何件どの順で作るかのワークフローは use ti-poc、メタデータ定義の配備は use ti-metadata。
version: 0.2.0
updated: 2026-07-25
---
...
| デモ/サンプルの自動作成チェックボックス連鎖生成【機構】 | どのシナリオを何件・どの順で・どんな体験で作るかの**ワークフロー**＝ ti-poc |
...
| デモ/PoC/テストデータを自動作成連鎖で作る | **ti-data-load** | 連鎖生成の機構 |
...
| デモ/サンプル/テストのトランザクションを自動作成連鎖で作る | `references/demo-chain-generation.md` | 起点（見積/受注）・自動作成フラグ・連鎖の型・非同期ドレイン待ち |
...
| reference | 何を定義するか |
|---|---|
| `references/migration-principles.md` | 移行の4本柱（会計計上レコードは自動作成＝個別名で列挙）／tb_ExId 関係解決（親→子→孫）／サンドボックス間多段移行／標準フロー（導入・デモ共通・期首残高主義） |
| `references/demo-chain-generation.md` | 自動作成チェックボックス連鎖の機構／function_only 起点（見積・受注）／連鎖の型／非同期ドレイン待ち／UI専用工程の system-mode 暫定策（2系統）／型は recipe を引く／本番非適用 |
...
- **機構はここ、型は ti-reference、段取りは ti-poc**。連鎖の機械（どう作るか）は本スキル、業務モデルの型（何を作るか）は ti-reference の recipe を索引で引く。機械の正しさ ≠ 型の正しさ。
