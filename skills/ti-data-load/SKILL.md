...
| **シームが無く明細を API で作る直前** | `references/migration-principles.md` §画面が写す項目を API 経路は肩代わりしない | 画面で作った1件と API で作った1件の差分・明示セットする項目・税抜/税込のどちらが後工程の入力か |
...
| reference | 何を定義するか |
|---|---|
| `references/migration-principles.md` | 移行の4本柱（会計計上レコードは自動作成＝個別名で列挙）／tb_ExId 関係解決（親→子→孫・**二段構えの引き当て**・送るレコードに自分自身の外部IDを含めない・**閉包を参照グラフから取る**）／サンドボックス間多段移行／標準フロー（導入・デモ共通・期首残高主義・**取引先の3段**・転記専用項目・再実行の担保）／**自動作成シーム経由で下流を生成する機構**（シーム経由の原則・手順5段・非同期ドレイン待ち・射程）／**画面が写す項目を API 経路は肩代わりしない** |
...
- **直 INSERT 原則禁止＝シーム経由**。書込直前に ti-reference の write-index を引き、判定（❌/⚠️/✅）に従う。ゲート機構は再実装しない。**シームが無く API で作るときは、画面が写す項目を API が肩代わりしないことを前提に置く**（`references/migration-principles.md` §画面が写す項目を API 経路は肩代わりしない）。
