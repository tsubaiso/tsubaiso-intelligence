...
ECA の OAuth（コンシューマ鍵・`oauthLink`）はプラットフォームが採番する。**4 つの構成要素（アプリ本体・OAuth 設定・グローバル OAuth 設定・ポリシー）を 1 回のデプロイにまとめて送れば、メタデータデプロイで新規作成できる**（2026-08-01 実測）。採番される 2 つの値は雛形から除去して送る。別 org の雛形を使う場合は `orgScopedExternalApp` を対象 org へ書き換える。
...
デプロイが通らないときは本手順の Setup UI で作成する。
