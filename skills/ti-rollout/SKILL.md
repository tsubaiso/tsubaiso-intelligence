...
| 役割 | タイミング | 案内すること |
|---|---|---|
| 管理者（Owner） | 導入時に一度（組織全体の土台づくり） | §3 全体（組織コネクタ登録・MCPサーバー有効化・接続用アプリ自動作成・接続キー・読み書き選択・プラグイン導入・組織セットアップ） |
| 各利用者 | 随時（本人が使い始めるとき） | コネクタ一覧で「接続」→本人ログインのみ（§3 step 6 相当・**追加や鍵設定は不要**） |

**判定**: 発火時に「初回の組織セットアップ（管理者）」か「各自の接続（利用者）」かを確認する。利用者フローは 1 手（接続＋本人ログイン）で短く済ませ、管理者フローは §3 を順に進める。役割が不明なら確認してから該当フローへ入る。§0 の個人プラン注意は役割に関わらず最初に提示する。

## 2. AI が自動で行う／人が本人操作する（境界）

| 区分 | 内容 |
|---|---|
| **AI が自動**（下書き・設定・案内） | 組織の Salesforce(PSA/IMA) 認証の開始、接続用アプリ（外部クライアントアプリ）の作成、接続キー取得、データ構造マップ生成、カスタマイズ設定の生成、各手順の画面案内 |
| **人が本人操作**（本人確認・権限） | 組織コネクタ登録、MCPサーバー有効化トグル、各種ログイン認可、読み取り/更新の選択、GitHubアクセス付与、書込・org反映の承認 |
...
4. **組織を Salesforce CLI に認証**（AI: CLI 用意・実行開始／人: ブラウザで本人ログインのみ・一度きり）。目的＝接続用アプリの自動作成をローカル実行で行う土台。
5. **接続用アプリ（外部クライアントアプリ）の作成＋接続キー取得**（AI 自動: メタデータ生成→検証→デプロイ→鍵取得）。目的＝最小権限の既定値で正しく設定し、手作業の設定ミス・過剰権限を防ぐ。**このアプリが標準 MCP 接続の OAuth 認証の受け皿**になり、取得した接続キー（Client ID）を step 6 でコネクタに設定する。OAuth の具体構成（コールバック・範囲・PKCE 等）は `references/api-eca-setup.md` が持つため本節では再掲しない（メタデータの生成・デプロイという機構そのものは ti-metadata）。**書込（org へのデプロイ）は承認ゲートを通す**（発火点表の ti-core safety-gate＝承認ドラフト提示→人が承認）。
...
9. **TIプラグイン／スキルの導入**（人: public マーケットプレイスを登録して取得。登録に招待・認証・追加課金は要らず、一度登録すれば以後の改訂・スキル追加を受け取れる。利用可否を決めるのは配布物へのアクセスではなく Tsubaiso Atlas MCP からの意味定義配信。他クライアントは形式変換）。
...
- 使えなくなること: Atlas／Knowledge 配信停止・プラグイン更新停止＝TI 支援機能は継続しない。
