...
5. **接続用アプリ（外部クライアントアプリ）の作成＋接続キー取得**（AI 自動: メタデータ生成→検証→デプロイ→鍵取得）。目的＝最小権限の既定値で正しく設定し、手作業の設定ミス・過剰権限を防ぐ。**このアプリが標準 MCP 接続の OAuth 認証の受け皿**になり、取得した接続キー（Client ID）を step 6 でコネクタに設定する。OAuth の具体構成（コールバック・範囲・PKCE 等）は ti-metadata が持つため本スキルでは再掲しない。**書込（org へのデプロイ）は承認ゲートを通す**（発火点表の ti-core safety-gate＝承認ドラフト提示→人が承認）。
...
9. **TIプラグイン／スキルの導入**（人: 契約者向け配布から取得。他クライアントは形式変換）。
